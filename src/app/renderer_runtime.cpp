...
#include "scene/loading_screen.h"
...
namespace chr {
    ...
    void run_render_loop(
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            constexpr float FRAME_TARGET_MS = 1000.0f / 60.0f;
            if (delta_time < 1.0f / 60.0f) {
                const float remaining_time_ms = FRAME_TARGET_MS - delta_time * 1000.0f;
                const auto sleep_ms = static_cast<int>(remaining_time_ms * 0.9f);
                if (sleep_ms > 0) {
