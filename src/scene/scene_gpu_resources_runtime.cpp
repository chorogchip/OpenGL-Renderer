...
namespace {
    ...
    static std::string make_texture_cache_key(
        ...
        const graphics::TextureColorSpace color_space) {
        const char* color_space_name =
            color_space == graphics::TextureColorSpace::Srgb ? "srgb" : "linear";
        return path + "::" + color_space_name;
    }

    static bool init_scene_gpu_shaders(chr::SceneGPUResources* resources) {

        unsigned shader_program = graphics::create_shader_program_from_files(
        ...
        resources->shadow_shader_program = graphics::create_shader_program_from_files(
    ...
    static bool upload_texture_when_decoded(
        ...
        if (prefetch_it != state->prefetch_futures.end()) {
            ...
            graphics::TextureImage image = future.get();
            state->prefetch_futures.erase(prefetch_it);
            *texture = graphics::create_texture_2d(image, color_space);
            if (*texture != 0) {
        ...
        if (!state->pending_texture_decode.valid()) {
            ...
            state->pending_texture_decode = std::async(
                std::launch::async,
                [path]() {
                    return graphics::load_texture_image(path);
                });
            state->message = "Decoding material texture...";
            return false;
        }
        ...
        graphics::TextureImage image = state->pending_texture_decode.get();
...
namespace chr {
        ...
        if (state->phase == SceneGPUInitPhase::TexturePrefetch) {
                ...
                for (const auto& path : unique_texture_paths) {
                    if (state->texture_cache.find(path) == state->texture_cache.end()) {
                        state->prefetch_futures[path] = std::async(
                            std::launch::async,
                            [path]() { return graphics::load_texture_image(path); });
                    }
                }
            ...
            for (auto& [path, future] : state->prefetch_futures) {
                ...
                if (future.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
                    ...
                    graphics::TextureColorSpace color_space = graphics::TextureColorSpace::Linear;
                    if (path.find("BaseColor") != std::string::npos ||
                        path.find("Emissive") != std::string::npos) {
                        color_space = graphics::TextureColorSpace::Srgb;
                    }

                    uint32_t texture = graphics::create_texture_2d(image, color_space);
                    if (texture != 0) {
        ...
        if (state->phase == SceneGPUInitPhase::Materials) {
            ...
            if (state->material_texture_step == 0) {
                if (!upload_texture_when_decoded(
                    ...
                    graphics::TextureColorSpace::Srgb,
                ...
                if (!upload_texture_when_decoded(
                    ...
                    graphics::TextureColorSpace::Linear,
    ...
    int reload_scene_gpu_shaders(SceneGPUResources* resources) {
        ...
        const uint32_t new_shadow_shader_program = graphics::create_shader_program_from_files(
